...
class ChatBotBloc extends BaseBloc<PageAction, PageEvent, PageState> {
  ...
  final _addEventchatBotUseCase = GetIt.I<AddEventChatbotUsecase>();
  ...
  ChatBotBloc() : super(PageState()) {
    ...
    on<EventDetailMessageAIChange>(_handleEventMessageAiChange);
    on<EventAddToCalendar>(_handleEventAddToCalendar);
    _initialize();
  }
  ...
  Future<void> _handleUserSendMessage(EventUserSendMessage event, Emitter emit) async {
    final updateListMessage = state.historyMessage?.message ?? []
      ..add(
        MessageUserAndAi(text: event.message, reply: ''),
      );
    emit(state.copyWith(
        historyMessage: HistoryMesssage(message: updateListMessage), loadingAiReply: true));
    final result = await _userChatUsercase
        .call((id_user: _sessionUseCase.userId ?? '', message: event.message));
    result.when(success: (data) {
      final MessageUserAndAi messageAireply = data.message.last;
      final updateListMessage = state.historyMessage?.message ?? []
        ..add(MessageUserAndAi(text: '', reply: messageAireply.reply));
      addAction(ActionSendMessageSuccess());
      emit(state.copyWith(
          historyMessage: HistoryMesssage(message: updateListMessage), loadingAiReply: false));
    }, failure: (error) {
      addAction(ActionSendMessageFaild(message: error.errorMessage));
    });
  }

  Future<void> _handleEventMessageAiChange(EventDetailMessageAIChange event, Emitter emit) async {
    emit(state.copyWith(messageAI: event.messageAi));
  }

  Future<void> _handleEventAddToCalendar(EventAddToCalendar event, Emitter emit) async {
    emit(state.copyWith(ishowLoading: true));
    final result = await _addEventchatBotUseCase.call((message: state.messageAI));
    result.when(success: (data) {
      emit(state.copyWith(ishowLoading: false));
      addAction(ActionAddCalendarSuccess());
    }, failure: (error) {
      emit(state.copyWith(ishowLoading: false));
      addAction(ActionAddCalendarFaild(messsage: error.errorMessage));
    });
  }
}
